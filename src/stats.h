   ...
   public:
    ...
    void print() {
        ...
        std::cout << std::format("nprobe:{} opt_level: {} simi_ratio: {}\n", nprobe, static_cast<int>(opt_level), simi_ratio)
                  << std::format("tri: {}({:.2f}%) tri_large: {}({:.2f}%) subnn_L2: {}({:.2f}%) subnn_IP: {}({:.2f}%) pivot_skip: {}\n", skip_triangle_count, pruning_triangle, skip_triangle_large_count, pruning_triangle_large, skip_subnn_L2_count, pruning_subnn_L2, skip_subnn_IP_count, pruning_subnn_IP, skip_pivot_count)
    ...
    void toCsv(std::string filename, bool append, std::string dataset = "Unknown") {
        CsvWriter writer(filename,
                         {"dataset", "nlist", "nprobe", "opt_level", "simi_ratio",
                          "tri", "tri_large", "subnn_L2", "subnn_IP", "pivot_skip", "simi_update_rate",
                          "check_L2", "check_IP",
        ...
        writer << dataset << nlist << nprobe << static_cast<int>(opt_level) << simi_ratio
               << skip_triangle_count << skip_triangle_large_count << skip_subnn_L2_count << skip_subnn_IP_count << skip_pivot_count << simi_update_rate / 100
